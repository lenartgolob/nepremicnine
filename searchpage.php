...
<table class="table table-hover" id="miza">
  ...
  <tbody>
    ...
    $query = "SELECT n.*, s.slika, k.ime AS ime_kraja FROM nepremicnine n INNER JOIN slike s ON n.id = s.nepremicnina_id INNER JOIN kraji k ON k.id = n.kraj_id WHERE lower(n.ime) LIKE '%$search%' OR lower(n.posredovanje) LIKE '%$search%' OR lower(n.lokacija) LIKE '%$search%' OR lower(k.ime) LIKE '%$search%' OR lower(n.naslov) LIKE '%$search%' OR lower(opis1) LIKE '%$search%' OR lower(opis2) LIKE '%$search%' ORDER BY id DESC";
